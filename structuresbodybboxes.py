...
class PersonAndFaceResult:
    ...
    def associate_body_bboxes(self, body_bboxes: List[Dict[str, Union[int, List[int]]]]):
        """
        Associa as bounding boxes do corpo, vindas do JSON, aos resultados existentes do YOLO.
        """
        for bbox in body_bboxes:
            bbox_tensor = torch.tensor(
                [bbox['bbox'][0], bbox['bbox'][1], bbox['bbox'][0] + bbox['bbox'][2], bbox['bbox'][1] + bbox['bbox'][3], 0, 1.0], 
                device=self.yolo_results.boxes.data.device
            )  # Move para o mesmo dispositivo

            if bbox_tensor.shape[0] != self.yolo_results.boxes.data.shape[1]:
                raise ValueError(f"Dimensão da bbox inválida: {bbox_tensor.shape[0]} (esperado: {self.yolo_results.boxes.data.shape[1]})")

            # Concatenar a nova bounding box
            self.yolo_results.boxes.data = torch.cat((self.yolo_results.boxes.data, bbox_tensor.unsqueeze(0)), dim=0)
            self.yolo_results.names.update({len(self.yolo_results.names): 'person'})

            # Expandir as listas para incluir a nova bounding box
            self.ages.append(None)
            self.genders.append(None)
            self.gender_scores.append(None)

        # Recalcula os índices após adicionar as novas bboxes
        self.unassigned_persons_inds = self.get_bboxes_inds("person")
