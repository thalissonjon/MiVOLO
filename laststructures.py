...
class PersonAndFaceResult:
    def __init__(self, results: Results):
        ...
        assert "face" or "person" in names
    ...
    def merge_results(self, json_results: Results):
        """
        Combina os resultados do YOLO com os do JSON.
        """
        # Combina as caixas diretamente, se ambas são do mesmo tipo e formato
        self.yolo_results.boxes.data = torch.cat((self.yolo_results.boxes.data, json_results.boxes.data), dim=0)
        print(self.yolo_results.boxes.data)
        
        # Atualiza os nomes para garantir que as classes estejam corretas
        self.yolo_results.names.update(json_results.names)
        
        # Atualiza as listas de idades, gêneros, etc.
        self.ages.extend([None] * len(json_results.boxes))
        self.genders.extend([None] * len(json_results.boxes))
        self.gender_scores.extend([None] * len(json_results.boxes))

        # Atualiza as IDs para garantir que não há duplicação (caso as IDs sejam usadas)
        if self.yolo_results.boxes.id is not None and json_results.boxes.id is not None:
            current_max_id = self.yolo_results.boxes.id.max().item() + 1
            json_results.boxes.id += current_max_id

        # Mantém a consistência nos índices de pessoas não atribuídas
        self.unassigned_persons_inds.extend(self.get_bboxes_inds("person"))
    ...
    def get_results_for_tracking(self) -> Tuple[Dict[int, AGE_GENDER_TYPE], Dict[int, AGE_GENDER_TYPE]]:
        ...
        for _, (det, age, gender, _) in enumerate(zip(pred_boxes, self.ages, self.genders, self.gender_scores)):
            ...
            name = self.yolo_results.names[cat_id]
            if name == "person":
