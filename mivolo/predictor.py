...
class Predictor:
    ...
    def recognize(self, image: np.ndarray, roi = None) -> Tuple[PersonAndFaceResult, Optional[np.ndarray]]:
        if roi:
            x, y, w, h = roi
            frame_cropped = image[y:y+h, x:x+w]
        else:
            frame_cropped = image
        ...
        if self.draw:
            try:
                ...
                if roi:
                    image[y:y+h, x:x+w] = annotated_cropped_frame
                else:
                    image = annotated_cropped_frame
...
#         detected_objects: PersonAndFaceResult = self.detector.predict(image)
#         self.age_gender_model.track(image, detected_objects)
